...
import { Board, Card, Label } from '@ngs/modules/data-access-scrumboard';
import { ScrumboardFacade } from '../../../services/scrumboard.facade';
import { Router } from '@angular/router';
import { MatTooltipModule } from '@angular/material/tooltip';

@Component({
  selector: 'feature-scrumboard-card-details',
...
export class ScrumboardCardDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private _scrumboardFacade: ScrumboardFacade,
  ...
  ngOnInit(): void {
    ...
    this._scrumboardFacade
      .getCard(this._router.url.split('card/')[1])
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((card) => {
        this.card = card!;
    ...
    this.cardForm.valueChanges
      ...
      .subscribe((value) => {
        this._scrumboardFacade.updateCard(value.id, {
          ...value,
  ...
  deleteCard() {
    this._scrumboardFacade.deleteCard(this.card.id ?? '');
   ...
   * @param event
   */
  filterLabelsInputKeyDown(event: any): void {
